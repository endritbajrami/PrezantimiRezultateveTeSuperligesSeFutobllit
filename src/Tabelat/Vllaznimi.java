...
public class Vllaznimi {
	...
	public Vllaznimi(int eid, int Numri, String Shtetesia, String Pozicioni,String EmriMbiemri) {
		this.eid = eid;
		this.Numri = Numri;
	...
	public static List<Vllaznimi> getvllaznimi() {
		List<Vllaznimi> vllaznimiList = new ArrayList();
		
		String query = "Select * from vllaznimi";
		...
		try {
			...
			while(resultSet.next()) {
				Vllaznimi vllaznimi = new Vllaznimi(resultSet.getInt(1), resultSet.getInt(2), resultSet.getString(3), resultSet.getString(4), resultSet.getString(5));
				vllaznimiList.add(vllaznimi);
			}
		} catch(SQLException ex) {
			ex.printStackTrace();
		}
		
		return vllaznimiList;
	}
}
